  ...
  return (
    <>
      <div className='px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw]'>    
        ...
        {/* Store Info */}
        <div className="my-10 flex flex-col md:flex-row gap-10 mb-28 items-start md:items-center">
          <img src={assets.contact_img} className='w-full md:max-w-[480px] rounded-xl' />
          <div className="flex flex-col justify-center items-start gap-6 flex-1">
            ...
            <p className='font-semibold text-xl text-gray-600'>Careers At QuickBite</p>
            <p className='text-gray-500'>Interested in joining our team network.</p>
            <button className='border border-black px-6 py-3 sm:px-8 sm:py-4 text-sm hover:bg-black hover:text-white transition-all duration-500'>Explore Jobs</button>
          </div>
        </div>

        {/* Heading */}
        <div className='flex flex-col justify-center items-center mb-28 text-center px-2 sm:px-0'>
          <h1 className='text-gray-950 text-4xl sm:text-5xl font-semibold font-mono'>Let's talk over</h1>
          <h1 className='text-[#FF6D1F] text-4xl sm:text-5xl font-semibold font-mono mt-2 sm:mt-4'>coffee.</h1>
          <p className='mt-3 sm:mt-5 text-center text-gray-600'>Whether you want to book a large event, share a family recipe, or just say hello, our door is always open.</p>
        </div>

        {/* Contact Form + Info */}
        <div className='flex flex-col md:flex-row justify-center items-start gap-8 md:gap-10 mb-24'>

          {/* Left Column: Form */}
          {isSubmitted ? (
            <div className='w-full md:w-1/2 bg-white rounded-xl p-6 sm:p-8 shadow-md flex flex-col items-center gap-6 text-center'>
              <img src={assets.mail} alt='' className='w-48 sm:w-64' />
              <h2 className='text-2xl sm:text-3xl font-semibold text-[#FF6D1F]'>Message Sent Successfully!</h2>
              <p className='text-gray-600'>Thank you for reaching out. We’ll get back to you shortly.</p>
              <button onClick={() => setIsSubmitted(false)} className='bg-[#FF6D1F] text-white rounded-xl px-6 py-2 sm:px-6 sm:py-3 hover:bg-[#EA580C] transition-colors font-semibold'>Send Another Message</button>
            </div>
          ) : (
            <form onSubmit={onSubmitHandler} className='w-full md:w-1/2 bg-white rounded-xl p-6 sm:p-8 shadow-md flex flex-col gap-4'>
              <h2 className='text-2xl sm:text-3xl font-semibold text-[#FF6D1F] mb-[-5px]'>Send us a note</h2>
              <p className='text-gray-600'>We read every message with our morning espresso. Share your thoughts or book a table.</p>

              <div className='flex flex-col sm:flex-row gap-4'>
                <div className='flex-1 flex flex-col'>
                  <label className='text-[#FF6D1F] font-semibold mb-1'>YOUR NAME</label>
                  <input type='text' onChange={(e) => setName(e.target.value)} value={name} placeholder='Your Name' className='border border-gray-300 rounded-xl px-3 sm:px-4 py-2 focus:outline-none focus:border-[#FF6D1F]' required />
                </div>

                <div className='flex-1 flex flex-col'>
                  <label className='text-[#FF6D1F] font-semibold mb-1'>PHONE</label>
                  <input type='tel' onChange={(e) => setPhone(e.target.value)} value={phone} placeholder='Phone (Optional)' className='border border-gray-300 rounded-xl px-3 sm:px-4 py-2 focus:outline-none focus:border-[#FF6D1F]' />
                </div>
              </div>

              <div className='flex flex-col'>
                <label className='text-[#FF6D1F] font-semibold mb-1'>EMAIL ADDRESS</label>
                <input type='email' onChange={(e) => setEmail(e.target.value)} value={email} placeholder='Your Email' className='w-full border border-gray-300 rounded-xl px-3 sm:px-4 py-2 focus:outline-none focus:border-[#FF6D1F]' required />
              </div>

              <div className='flex flex-col'>
                <label className='text-[#FF6D1F] font-semibold mb-1'>YOUR MESSAGE</label>
                <textarea placeholder='Your Message' rows={5} onChange={(e) => setTextArea(e.target.value)} value={textArea} className='w-full border border-gray-300 rounded-xl px-3 sm:px-4 py-2 focus:outline-none focus:border-[#FF6D1F] resize-none'></textarea>
              </div>

              <button type='submit' className='bg-[#FF6D1F] text-white rounded-xl px-6 py-2 sm:px-6 sm:py-3 hover:bg-[#EA580C] transition-colors font-semibold'>Send Message</button>
            </form>
          )}

          {/* Right Column: Info Panel */}
          <div className='w-full md:w-80 h-auto md:h-[604px] bg-[#FF6D1F] rounded-xl p-6 sm:p-8 flex flex-col gap-8'>
            <div>
              <h3 className='text-2xl font-semibold text-white mb-3 sm:mb-10'>Come Visit Us</h3>
              <hr className='mt-0 mb-5 sm:mt-[-15px] sm:mb-12' />
              <p className='text-black text-xl sm:text-2xl font-semibold'>Our Kitchen</p>
              <p className='text-white text-sm sm:text-base'>123 Maple Avenue, Sunnyville, CA 90210</p>
            </div>

            <div>
              <h3 className='text-2xl font-semibold text-black mb-1 sm:mb-2'>Give Us a Ring</h3>
              <p className='text-white text-sm sm:text-base'>(555) 867-5309</p>
            </div>

            <div>
              <h3 className='text-2xl font-semibold text-black mb-1 sm:mb-2'>Opening Hours</h3>
              <p className='text-white text-sm sm:text-base'>Mon - Fri: 8am - 10pm</p>
              <p className='text-white text-sm sm:text-base'>Sat - Sun: 9am - 11pm</p>
            </div>
          </div>
