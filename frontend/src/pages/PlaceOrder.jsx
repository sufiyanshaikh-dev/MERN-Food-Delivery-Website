...
const PlaceOrder = () => {
  const { navigate, backendUrl, token, cartItems, setCartItems, getCartAmount, delivery_fee, products } = useContext(FoodContext)
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      const orderData = {
        ...
        userId,
        paymentMethod: method // <-- added to fix schema error
      }
      ...
      if (method === "cod") {
        ...
        if (response.data.success) {
          setCartItems({});
          navigate("/orders"); // redirect works now
        } else {
      ...
      // --------------------- RAZORPAY METHOD ----------------------
  //     if (method === "razorpay") {
  //       toast.info("Razorpay payments are not supported.");
  //       return;
  //     }
      ...
      <div className="mt-8">
        ...
        <div className="mt-12">
          ...
          <div className="w-full text-end mt-8 mb-10">
            <button type='submit' className='bg-black text-white px-16 py-3 text-sm rounded-lg mr-12'>PLACE ORDER</button>
          </div>
        </div>
      </div>
    </form>
