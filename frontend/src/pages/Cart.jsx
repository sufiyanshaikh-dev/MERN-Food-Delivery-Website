...
const Cart = () => {
  ...
  return (
    <>
      <div className='px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw] z-10 mt-6'>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full px-4 sm:px-[5vw] md:px-[7vw] lg:px-[9vw] z-50">
        <div className="w-full bg-gray-100 shadow-xl rounded-t-xl p-5">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="border-t pt-3 flex justify-between font-semibold text-lg text-gray-900">
              <span>Total</span>
              <span>₹{getCartAmount() > 0 ? getCartAmount() + 40 : 0}</span>
            </div>
          </div>

          {/* CHECKOUT BUTTON */}
          <button onClick={() => navigate("/place-order")} className="mt-5 w-full bg-[#FF6D1F] text-white py-3 rounded-xl font-semibold hover:bg-[#e55b1c] transition-colors duration-300">
            Proceed to Checkout
          </button>
        </div>
      </div>
